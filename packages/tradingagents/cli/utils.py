...
TICKER_INPUT_EXAMPLES = "SPY, 0700.HK, BTC-USD, RELIANCE.NS, NIFTY"
INDIA_TICKER_INPUT_EXAMPLES = "RELIANCE, NIFTY, BANKNIFTY-FUT, NIFTY26SEP23500CE, GOLD, USDINR"
...
ANALYST_ORDER = [
    ...
    ("Derivatives (F&O) Analyst", AnalystType.DERIVATIVES),
]

CRYPTO_SUFFIXES = ("-USD", "-USDT", "-USDC", "-BTC", "-ETH")

# Asset types with no company or index behind them, so no fundamentals.
_NO_FUNDAMENTALS = frozenset({AssetType.CRYPTO, AssetType.COMMODITY, AssetType.CURRENCY})


def ticker_examples() -> str:
    """Ticker examples for the prompt, Indian ones when the market preset is India."""
    from tradingagents.dataflows.india.instruments import is_india_mode

    return INDIA_TICKER_INPUT_EXAMPLES if is_india_mode() else TICKER_INPUT_EXAMPLES


def default_ticker() -> str:
    """Ticker used when the prompt is left empty: NIFTY in India mode, else SPY."""
    from tradingagents.dataflows.india.instruments import is_india_mode

    return "NIFTY" if is_india_mode() else "SPY"
...
def is_valid_ticker_input(value: str) -> bool:
    ...
    like ``GC=F`` and ``EURUSD=X`` (#980), ``^`` for indices, and ``&`` for NSE
    symbols like ``M&M``. Empty input is allowed (it defaults to SPY downstream).
    """
    v = value.strip()
...
def get_ticker() -> str:
    ...
    ticker = questionary.text(
        f"Enter ticker symbol (e.g. {ticker_examples()}):",
        validate=lambda x: (
            is_valid_ticker_input(x)
            or "Please enter a valid ticker symbol, e.g. AAPL, 000404.SZ, 0700.HK, GC=F, RELIANCE.NS, NIFTY26SEPFUT."
        ),
...
def normalize_ticker_symbol(ticker: str) -> str:
    """Resolve user input to the canonical symbol the pipeline carries.
    ...
    (e.g. ``BTCUSD`` -> ``BTC-USD``, ``XAUUSD`` -> ``GC=F``). Indian instruments
    keep their own symbol (``NIFTY26SEPFUT``, ``GOLD.MCX``, ``RELIANCE.NS``): the
    data layer prices them from their underlying or proxy, while the derivatives
    tools and instrument context need the contract itself. Falls back to the
...
def detect_asset_type(ticker: str) -> AssetType:
    ...
    crypto (#981/#982), matching what the data path will actually fetch. Indian
    instruments map to their segment (cash equities stay ``stock``)."""
    from tradingagents.dataflows.india.instruments import asset_type_for, parse_indian_symbol

    indian = parse_indian_symbol(ticker)
    if indian is not None:
        return AssetType(asset_type_for(indian))
...
def filter_analysts_for_asset_type(
    ...
    """Drop analysts whose data cannot exist for the instrument.

    Fundamentals need a company or an index behind the symbol. The derivatives
    analyst needs an Indian instrument with exchange-traded F&O, so it is dropped
    ...
    if ticker:
        ...
        indian = parse_indian_symbol(ticker)
        supports_derivatives = indian is not None and indian.supports_exchange_derivatives
    return [
        analyst
        for analyst in analysts
        if not (analyst == AnalystType.FUNDAMENTALS and asset_type in _NO_FUNDAMENTALS)
        and not (analyst == AnalystType.DERIVATIVES and not supports_derivatives)
    ]
