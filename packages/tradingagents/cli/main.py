...
class MessageBuffer:
    ...
    def _update_current_report(self):
        ...
        if latest_section and latest_content:
            ...
            section_titles = {
                ...
                "derivatives_report": "Derivatives (F&O) Analysis",
    ...
    def _update_final_report(self):
        ...
        analyst_sections = [
            "market_report", "sentiment_report", "news_report", "fundamentals_report", "derivatives_report",
        ]
...
def get_user_selections():
    ...
    from tradingagents.dataflows.india.instruments import is_india_mode, parse_indian_symbol

    console.print(
        create_question_box(
            "Step 1: Ticker Symbol",
            (
                "Indian market mode: enter an NSE/BSE stock, index, F&O contract, MCX commodity or INR pair "
                "(e.g. RELIANCE, NIFTY, BANKNIFTY-FUT, NIFTY26SEP23500CE, GOLD, USDINR)"
                if is_india_mode()
                else "Enter the ticker, with exchange suffix when needed "
                "(e.g. SPY, 0700.HK, BTC-USD, RELIANCE.NS, NIFTY26SEPFUT)"
            ),
            default_ticker(),
        )
    )
    ...
    indian = parse_indian_symbol(selected_ticker)
    if indian is not None:
        # Show how the Indian instrument was understood before the run starts.
        details = [f"segment {indian.segment}", f"underlying {indian.underlying}", f"exchange {indian.exchange}"]
        if indian.expiry_label():
            details.append(indian.expiry_label())
        if indian.strike is not None:
            details.append(f"strike {indian.strike:g} {indian.option_type}")
        console.print(f"[green]Indian instrument:[/green] {selected_ticker} ({', '.join(details)})")
...
ANALYST_ORDER = ["market", "social", "news", "fundamentals", "derivatives"]
ANALYST_AGENT_NAMES = {
    "market": "Market Analyst",
    "social": "Sentiment Analyst",
    "news": "News Analyst",
