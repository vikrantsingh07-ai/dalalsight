...
from tradingagents.dataflows.india.commodities import download_proxy_ohlcv, is_proxy_symbol
from tradingagents.dataflows.india.fno import resolve_contract
...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        Args:
            selected_analysts: List of analyst types to include ("market",
                "social", "news", "fundamentals", and "derivatives" for Indian
                instruments with exchange-traded F&O)
            debug: Whether to run in debug mode
            config: Configuration dictionary. If None, uses default config
            callbacks: Optional list of callback handlers (e.g., for tracking LLM/tool stats)
        """
        self.debug = debug
        # A programmatic ``config["market"] = "india"`` gets the same preset the
        # TRADINGAGENTS_MARKET env var applies to DEFAULT_CONFIG.
        self.config = apply_market_preset(config or DEFAULT_CONFIG)
    ...
    def _create_tool_nodes(self) -> dict[str, ToolNode]:
        ...
        The Indian tools are registered unconditionally; analysts bind them to
        the LLM only for Indian instruments, so other runs never call them.
        """
        return {
            "market": ToolNode(
                [
                    ...
                    get_verified_market_snapshot,
                    # NSE delivery percentage (Indian stocks)
                    get_delivery_analysis,
                ]
            ),
            "social": ToolNode(
            ...
            "news": ToolNode(
                [
                    ...
                    get_macro_indicators,
                    get_prediction_markets,
                    # FII/DII cash-market flows (Indian instruments)
                    get_fii_dii_cash_flows,
                ]
            ),
            "fundamentals": ToolNode(
                [
                    ...
                    get_cashflow,
                    get_income_statement,
                    # NSE corporate actions, results calendar, filings
                    get_corporate_events,
                ]
            ),
            "derivatives": ToolNode(
                [
                    # Indian F&O positioning from exchange EOD files
                    get_fno_futures_analysis,
                    get_option_chain_analysis,
                    get_fii_derivatives_positioning,
                    get_india_vix,
    ...
    def _resolve_benchmark(self, ticker: str) -> str:
        ...
        ``config["benchmark_ticker"]`` overrides everything when set. Indian
        instruments (including F&O contracts, MCX commodities and INR pairs,
        which carry no exchange suffix) are measured against Nifty 50, or the
        Sensex for BSE listings and BSE index derivatives. Otherwise the suffix
        map matches the ticker's exchange suffix (e.g. ``.T`` for Tokyo).
        US-listed tickers without a dotted suffix fall through to the
        ...
        indian = parse_indian_symbol(ticker)
        if indian is not None:
            if indian.exchange == "BSE":
                return benchmark_map.get(".BO", "^BSESN")
            return benchmark_map.get(".NS", "^NSEI")
    ...
    def _fetch_returns(
        ...
        or unreachable. Indian F&O contracts are settled on their underlying.
        """
        ...
        try:
            ...
            if is_proxy_symbol(canonical):
                stock = download_proxy_ohlcv(canonical, trade_date, end_str)
            else:
    ...
    def resolve_instrument_context(
        ...
        graph regardless of entry point. With ``trade_date``, Indian futures and
        options also get their expiry and lot size from the exchange bhavcopy.
        """
        identity = resolve_instrument_identity(ticker)
        contract = resolve_contract(ticker, str(trade_date)) if trade_date else None
        return build_instrument_context(ticker, asset_type, identity, contract)
    ...
    def propagate(self, company_name, trade_date, asset_type: str = "stock"):
        ...
        ``asset_type`` selects between the stock pipeline (default), the
        crypto pipeline (``"crypto"``) shipped in #567, and the Indian segments
        (``"index"``, ``"futures"``, ``"options"``, ``"commodity"``,
        ``"currency"``) — the CLI auto-detects from the ticker; programmatic
        callers pass it explicitly. When
