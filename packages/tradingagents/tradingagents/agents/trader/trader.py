...
def create_trader(llm):
    ...
    def trader_node(state, name):
        ...
        # Indian F&O runs: futures prices, option premiums, strikes and lot sizes
        # live in the derivatives report, not the underlying's technical report.
        derivatives_report = (state.get("derivatives_report") or "").strip()
        if derivatives_report:
            grounding += (
                "For futures and options, take contract-level figures (futures price, option "
                "premium, strikes, expiry, lot size) from the derivatives report. "
            )
            report_section += f"Derivatives (F&O) Report:\n{derivatives_report}\n\n"
