...
# Upstream overload / rate-limit reported inside an HTTP 200 body by routers such
# as OpenRouter ("Upstream error from Nvidia: Service temporarily overloaded",
# code 502). langchain raises ValueError for these, which the SDK's own retry
# budget never sees.
_TRANSIENT_UPSTREAM_ERROR = re.compile(
    r"'code':\s*(429|50[0-9])|overloaded|provider_unavailable|temporarily unavailable",
    re.IGNORECASE,
)


def _upstream_retries() -> int:
    try:
        return max(0, int(os.environ.get("TRADINGAGENTS_UPSTREAM_RETRIES", "3")))
    except ValueError:
        return 3
...
class NormalizedChatOpenAI(ChatOpenAI):
    ...
    def invoke(self, input, config=None, **kwargs):
        attempts = _upstream_retries() + 1
        for attempt in range(attempts):
            try:
                return normalize_content(super().invoke(input, config, **kwargs))
            except ValueError as exc:
                # A single overloaded upstream call used to abort a whole
                # multi-agent run; retry transient body-level errors with backoff.
                if attempt == attempts - 1 or not _TRANSIENT_UPSTREAM_ERROR.search(str(exc)):
                    raise
