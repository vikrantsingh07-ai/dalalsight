...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    write_cache = data is None
    if data is None:
        from .india.commodities import download_proxy_ohlcv, is_proxy_symbol

        if is_proxy_symbol(canonical):
            # MCX has no free feed: price the USD/INR-converted global benchmark.
            downloaded = download_proxy_ohlcv(canonical, start_str, end_str)
        else:
        ...
        data = None if downloaded.empty or "Close" not in downloaded.columns else downloaded

    # Some NSE index feeds on Yahoo are stale or nearly empty (NIFTY Financial
    # Services, NIFTY Midcap Select): rebuild them from NSE's official history.
    today_str = today_date.strftime("%Y-%m-%d")
    if needs_nse_index_fallback(canonical, data, start_str, today_str):
        nse_history = nse_index_history_frame(canonical, start_str, today_str)
        if nse_history is not None:
            data, write_cache = nse_history.reset_index(), True
