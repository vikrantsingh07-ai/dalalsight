...
_YAHOO_SAFE = re.compile(r"^[A-Za-z0-9._\-\^=&]+$")  # '&' for NSE symbols like M&M.NS
...
def normalize_symbol(raw: str) -> str:
    ...
    Resolution order (first match wins):
      0. Indian instruments (``india/instruments.py``): NSE/BSE indices, F&O
         contracts -> their underlying, MCX commodities -> a ``<CODE>.MCX``
         proxy, INR pairs, and bare NSE symbols when ``market`` is ``india``.
    ...
    # Lazy: india.instruments imports this module's alias tables.
    from .india.instruments import parse_indian_symbol

    indian = parse_indian_symbol(s)
    crypto = _normalize_crypto(s)
    if indian is not None:
        canonical = indian.price_symbol
    elif s in _ALIASES:
...
def reference_symbol(raw: str) -> str:
    """Real Yahoo symbol for news and identity lookups.

    Equal to :func:`normalize_symbol` except for synthetic price symbols: an MCX
    commodity prices from a ``GOLD.MCX`` proxy series, but its news lives under
    the global benchmark (``GC=F``).
    """
    from .india.instruments import parse_indian_symbol

    indian = parse_indian_symbol(raw) if isinstance(raw, str) else None
    return indian.reference_symbol if indian is not None else normalize_symbol(raw)
