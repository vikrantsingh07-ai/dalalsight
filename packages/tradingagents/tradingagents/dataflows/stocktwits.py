...
def _stocktwits_symbol(ticker: str) -> str | None:
    """Map a ticker to its StockTwits stream symbol, or None when there is none.

    StockTwits lists crypto as ``BTC.X`` (Yahoo's ``BTC-USD`` form 404s), so any
    crypto symbol resolves to its base plus ``.X``. NSE stocks, and F&O
    contracts on them, use ``<SYMBOL>.NSE``; other Indian instruments (indices,
    MCX commodities, INR pairs, BSE scrip codes) have no stream. Other symbols
    pass through upper-cased.
    """
    ...
    if indian is not None:
        if indian.underlying_kind == "stock" and indian.exchange == "NSE":
            return f"{indian.underlying}.NSE"
        return None
...
def fetch_stocktwits_messages(
    ...
    if symbol is None:
        return (
            f"<no StockTwits stream for {ticker.upper()}: StockTwits does not cover "
            f"this Indian instrument type>"
        )
