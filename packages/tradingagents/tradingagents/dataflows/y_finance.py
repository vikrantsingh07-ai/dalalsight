...
def get_YFin_data_online(
    ...
    from .india.commodities import download_proxy_ohlcv, is_proxy_symbol
    ...
    is_proxy = is_proxy_symbol(canonical)
    if is_proxy:
        # MCX commodity: no free feed, so price the USD/INR-converted benchmark.
        data = download_proxy_ohlcv(canonical, start_date, end_inclusive)
    else:
    ...
    source_note = ""
    if not is_proxy:
        from .india.index_history import needs_nse_index_fallback, nse_index_history_frame

        if needs_nse_index_fallback(canonical, data, start_date, end_date):
            nse_history = nse_index_history_frame(canonical, start_date, end_date)
            if nse_history is not None:
                data = nse_history
                source_note = "# Source: NSE index history (the Yahoo feed for this index is stale or incomplete)\n"
    ...
    if is_proxy:
        header += (
            "# MCX price proxy: global benchmark future converted at USD/INR into the MCX "
            "quote unit; excludes import duty and local premium, so levels differ from MCX quotes\n"
        )
    header += source_note
...
def get_cashflow(
    ...
    try:
        ...
        note = ""
        if data.empty and freq.lower() == "quarterly":
            # Yahoo publishes only annual cash flow for many Indian companies
            # (RELIANCE, HDFCBANK); fall back instead of reporting no data.
            data = filter_financials_by_date(yf_retry(lambda: ticker_obj.cashflow), curr_date)
            if not data.empty:
                freq = "annual"
                note = "# Quarterly cash flow is not published by the vendor for this company; showing annual statements.\n"
